...
public class CameraPOIExample {
	 ...
	 */
	public CameraPOIExample(int width, int height) {
		// generate and initialize Renderer 
		renderer = new Ogl3Renderer (width, height);
		renderer.initState();
		renderer.setTitle ("CameraPOIExample");
		
		// generate root element of the scene
		world = new Group ("CameraPOIScene");
		...
		CirclePosController pctrl = new CirclePosController("Circle", 2.0f, cube.getChannel("Translation"));
		
		// Add controller to controller array
		controllers.add(pctrl);
		
		// Create connection between position of cube and POI of camera
		ConnectorAddVec3 cctrl = new ConnectorAddVec3("Connect", 
														new Vec3(2.0f, 5.0f, 10.0f), 
														cube.getChannel("Translation"), 
														camera.getChannel("Translation"));
	...
	public static void main(String[] args) {
		...
		CameraPOIExample demo   = new CameraPOIExample( width, height );
		demo.gameLoop();
	}
}
