...
import animation.FollowerCameraController;
...
public class TestBezierKubisch
{
	...
	public TestBezierKubisch(int width, int height)
	{
		...
		// Zusammen setzen
			...
			float i = 0; // verzögerung beim start
			...
			FunctionR1Vec3 end = FunctionR1Vec3Util.connect(s1_prime,s2_prime);
			
			
		
		world = VisualHelp.makePath(world,end );
		
		
		controllers.add( new FunctionR1Vec3Controller(AbstController.RepeatType.CLAMP, bird.getChannel("Translation"), end));
		
		//Orentation
		controllers.add(new OrientationController("OrinetationController", end, bird.getChannel("Rotation")));
		
		//adding followerCamController
		FollowerCameraController camCont = new FollowerCameraController(camera.getChannel(AbstCamera.TRANSLATION),
				camera.getChannel(AbstCamera.ROTATION), end, 1f);
		controllers.add(camCont);
